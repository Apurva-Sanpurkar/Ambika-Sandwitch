...
import { 
  ...
  History, Search, Phone, Star, ChefHat, Package, Clock
} from 'lucide-react';
  ...
  const handleProfessionalShare = async () => {
    ...
    try {
      ...
      const message = `*🥪 AMBIKA SANDWICH 🥪*\n` +
        ...
        `✅ *TAP TO PAY VIA UPI:* \n${upiLink}\n\n` +
        `⏳ *Estimated Wait:* ${waitInfo.minutes} Mins\n` +
        `--------------------------------\n` +
        `*Visit Again!* ✨`;
    ...
    <div className="flex flex-col h-screen bg-[#F8F9FA] text-[#1A1A1A] overflow-hidden select-none font-sans">
      <header className="p-4 bg-white border-b flex justify-between items-center shadow-sm z-30">
        <div className="text-left">
          ...
          <p className="text-[8px] font-bold text-gray-400 uppercase tracking-widest mt-1">Captain Mobile v3.5</p>
        </div>
        <button onClick={() => setShowHistory(true)} className="p-2.5 bg-black text-white rounded-xl active:scale-90 transition-transform"><History size={20}/></button>
      </header>
      ...
      <main className="flex-grow overflow-y-auto p-4 pb-32">
        {Object.entries(MENU_DATA).map(([cat, items]) => (
          <div key={cat} className="mb-8 text-left">
            <h2 className="text-[10px] font-black uppercase text-gray-400 mb-4 flex items-center gap-2 tracking-widest"><Zap size={12} className="text-[#FFC107]"/> {cat.replace('_',' ')}</h2>
            <div className="grid grid-cols-2 gap-3">
              {items.map(item => (
                <button key={item.id} disabled={availability[item.id] === false} onClick={() => addToCart(item)}
                  className={`bg-white p-4 rounded-[2rem] border-b-4 flex flex-col items-start active:scale-95 ${availability[item.id] === false ? 'opacity-30 border-gray-200 pointer-events-none' : 'border-[#FFC107] shadow-sm'}`}>
      ...
      <AnimatePresence>
        {cart.length > 0 && !showCartMobile && (
          <motion.div initial={{ y: 100 }} animate={{ y: 0 }} className="fixed bottom-6 left-4 right-4 bg-black text-white p-4 rounded-[2rem] shadow-2xl z-40 flex items-center justify-between border-t border-white/10">
            <div className="pl-2 text-left"><span className="text-[9px] text-gray-400 uppercase font-black block">Total bill</span><span className="text-2xl font-black">₹{totalPrice}</span></div>
            <button onClick={() => setShowCartMobile(true)} className="bg-[#FFC107] text-black px-6 py-3 rounded-2xl font-black text-xs uppercase flex items-center gap-2">Review Cart <ShoppingCart size={16}/></button>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {showCartMobile && (
          <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'spring', damping: 25 }} className="fixed inset-0 bg-white z-[100] flex flex-col">
            ...
            <div className="flex-grow overflow-y-auto p-6 space-y-3 text-left">
              {cart.map((item, i) => (
                <div key={item.tempId} className="flex justify-between items-center p-4 bg-gray-50 rounded-[1.8rem] border border-gray-100">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col"><span className="font-black text-xs uppercase leading-none">{item.name}</span><span className="text-[10px] font-bold text-[#2E7D32] mt-1">₹{item.price}</span></div>
                  </div>
            ...
            <div className="p-8 bg-[#1A1A1A] text-white rounded-t-[3.5rem] shadow-2xl">
              ...
              <button onClick={handleCheckout} disabled={isProcessing} className="w-full py-5 bg-[#FFC107] text-black rounded-2xl font-black text-xl uppercase shadow-xl flex items-center justify-center gap-3 active:scale-95">{isProcessing ? "PROCESSING..." : <><CheckCircle size={24} strokeWidth={3}/> CONFIRM ORDER</>}</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {showPhoneModal && (
          <div className="fixed inset-0 bg-black/90 z-[200] flex items-center justify-center p-6 backdrop-blur-md">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-[2.5rem] p-8 w-full shadow-2xl text-left">
              <h2 className="text-2xl font-black uppercase italic mb-6">WhatsApp <span className="text-[#25D366]">Bill</span></h2>
              <div className="space-y-4">
                ...
                <div className="relative text-left">
                  ...
                  <button onClick={() => { setShowPhoneModal(false); setShowReceipt(true); }} className="absolute right-2 top-2 bg-[#25D366] text-white p-3 rounded-xl"><ArrowRight size={20} strokeWidth={3}/></button>
                </div>
                <div className="flex flex-wrap gap-2 mt-4 text-left">
                  {recentCustomers.map((c, i) => (
                    <button key={i} onClick={() => { setCustomerName(c.name); setCustomerPhone(c.phone); }} className="bg-gray-50 border border-gray-100 px-3 py-2 rounded-xl text-[9px] font-black flex items-center gap-2"><Star size={10} className="text-[#FFC107] fill-[#FFC107]"/> {c.name}</button>
                  ))}
                </div>
              </div>
            </motion.div>
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-white rounded-[2rem] p-6 w-full max-w-xs relative">
              ...
              <div ref={receiptRef} className="bg-white p-8 rounded-t-[2.5rem] border-b-4 border-dashed border-gray-100 font-mono text-black text-[10px] text-left">
                <div className="text-center mb-6">
                  <ChefHat className="mx-auto mb-2" size={30}/>
                  <h3 className="font-black text-lg uppercase italic tracking-tighter text-center">Ambika Sandwich</h3>
                  <p className="text-[7px] font-bold text-gray-400 uppercase mt-1 text-center tracking-widest">Fresh & Tasty Since 1998</p>
                </div>
                ...
                <div className="space-y-2 mb-6 uppercase font-bold text-[9px]">
                  {lastOrder?.items?.map((it, idx) => (
                    <div key={idx} className="flex justify-between uppercase"><span>{it.isParcel ? '📦' : '🍽️'} 1x {it.name.substring(0,18)}</span><span className="font-black">₹{it.price}</span></div>
                  ))}
                </div>
                <div className="flex justify-between font-black text-sm border-t-2 border-black pt-4 uppercase"><span>Total Amount</span><span>₹{lastOrder?.total}</span></div>
                <div className="mt-6 text-center text-[7px] text-gray-400 font-black uppercase tracking-widest text-center">Thank You • Visit Again</div>
              </div>

              <div className="bg-[#1A1A1A] p-4 rounded-b-[2.5rem] space-y-3 shadow-2xl">
                <button onClick={handleProfessionalShare} className="w-full bg-[#25D366] text-white py-4 rounded-xl font-black text-xs uppercase flex items-center justify-center gap-3 active:scale-95 shadow-lg border-b-4 border-green-800"><MessageCircle size={18} strokeWidth={3}/> Share Msg & Image</button>
      ...
      <AnimatePresence>
        {showHistory && (
          <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} className="fixed inset-0 bg-white z-[150] flex flex-col p-6 text-left">
            <div className="flex justify-between items-center mb-8"><h2 className="text-2xl font-black italic uppercase tracking-tighter">Order <span className="text-[#FFC107]">History</span></h2><button onClick={() => setShowHistory(false)} className="p-2 bg-gray-100 rounded-full"><X size={20}/></button></div>
            <div className="relative mb-6 text-left"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300" size={18}/><input type="text" placeholder="Search Token #" value={historySearch} onChange={(e) => setHistorySearch(e.target.value)} className="w-full bg-gray-50 border-2 border-gray-100 rounded-2xl py-4 pl-12 pr-4 font-black text-sm outline-none focus:border-[#FFC107]"/></div>
            <div className="flex-grow overflow-y-auto space-y-4">{filteredHistory.map((order) => (<div key={order.id} className="p-5 bg-gray-50 rounded-2xl border flex justify-between items-center text-left"><div className="text-left"><span className="font-black text-xl block text-left">#{order.tokenNo}</span><span className="text-[10px] font-bold text-gray-400 uppercase text-left">{formatDate(order.timestamp)} • ₹{order.total}</span></div><button onClick={() => { setLastOrder(order); setShowReceipt(true); }} className="bg-white px-4 py-2 rounded-xl text-[10px] font-black uppercase border-2 border-black active:bg-black active:text-white">View</button></div>))}</div>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
